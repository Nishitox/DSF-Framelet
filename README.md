...
`dsf:frame/format`は、`dsf:cache`を元に、使用された痕跡のあるframe領域を強制的に削除する関数です。原則としてframe関数は正常終了時に対象のframeを解放する設計になっていますが、異常終了などでframeが正常に解放されずに残留した場合にすべてのframeを一括で削除するための機能です。**使用中のframeであっても強制的に解放されるため、デバッグや初期化での使用を想定しています。**
